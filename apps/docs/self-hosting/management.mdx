...
The **Kinpal CLI** (`rallly.sh`) is the single entry point for day-to-day operations on a self-hosted instance. It ships with the [self-hosted stack](https://github.com/kinpalai/kinpal-selfhosted) and should be run from the stack directory (e.g. `/opt/rallly` for installer-based setups).
...
By default the stack tracks the `kinpalai/rallly:4` tag, so `update` will bring you the latest compatible `4.x` release. To pin to a specific version or follow a different major, set `KINPAL_IMAGE` in `.env` (see [Configuration](/self-hosting/configuration#advanced)).
