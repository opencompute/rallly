...
```sh
docker compose exec -T kinpal_db pg_dump -U postgres rallly | gzip > kinpal-backup.sql.gz
```

Copy `kinpal-backup.sql.gz` off the server (e.g. `scp` to your laptop). Do not skip this.
...
gunzip < /path/to/kinpal-backup.sql.gz | docker compose exec -T db psql -v ON_ERROR_STOP=1 -U postgres rallly
```
...
rm -rf /path/to/old/kinpal-selfhosted
